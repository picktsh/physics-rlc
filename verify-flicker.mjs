// grid + rotate stress check after anti-flicker fix
...
const edge = spawn(EDGE, [
  ...
  '--user-data-dir=' + process.env.TEMP + '\\qoder-edge-tmpC',
...
// simulate user rotation drag on the 3D canvas over ~2s (many pointermove steps)
const rotOk = await evalJS(`(async () => {
  const cv = [...document.querySelectorAll('canvas')].find((c) => !c.className.includes('border-dashed'))
  const r = cv.getBoundingClientRect()
  const cx = r.left + r.width / 2, cy = r.top + r.height / 2
  const fire = (t, x, y) => cv.dispatchEvent(new PointerEvent(t, { bubbles: true, cancelable: true, pointerId: 7, clientX: x, clientY: y, buttons: t === 'pointerdown' ? 1 : 0 }))
  fire('pointerdown', cx, cy)
  for (let i = 1; i <= 40; i++) {
    await new Promise((res) => setTimeout(res, 16))
    fire('pointermove', cx + Math.sin(i / 4) * 260, cy + Math.cos(i / 4) * 160)
  }
  fire('pointerup', cx, cy)
  await new Promise((res) => setTimeout(res, 1200))
  return 'done'
})()`, true)
console.log('rotate =>', rotOk)
...
console.log('grid(8x6 avg rgb after rotation, top->bottom):')
