...
// U 端始终留白点收束,合成端位置仍可辨;U_X 细虚线画在 U 后作辅助;刻度仅外圈,端点仅 U 留点
function drawPhasor(pv) {
  ...
  // U_X = U_L + U_C 净电抗压降(紫细虚线,从 UR 端垂直伸出到 U 端,与 UR 构成直角求和链;
  // 仅当 UR 端与圆心可分(lr>2.6)时绘制——低/高频 UR≈0 时 UX≈U 与白线贴合,让位不画;
  // 谐振 |ll-lc|≤1.2 抵消后也不画)
  if (lr > 2.6 && Math.abs(ll - lc) > 1.2) drawVec(a, b, 'rgba(183,156,255,0.82)', 1.4, [4, 4])
