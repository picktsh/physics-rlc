 ...
 * 拓扑同为「信号源 V → R → L → C → 回 V」单回路，免去每次手动拖放 + 接线。connections 用 [元件下标，端点下标 (0 左/1 右), 元件下标，端点下标] 描述连线对。
 *
 * 坐标口径：矩形回路，画布坐标 x∈[60,280] y∈[60,140],适配 2D 画布;3D 场景经 ctx 自动居中缩放，不受绝对值影响。
 * 阻尼量级 (L=0.1H、C=0.1μF=1e-7F):ω₀=1/√(LC)=1e4 rad/s,临界 R=2√(L/C)=2000Ω。
 */
export const CIRCUIT_PRESETS = [
  {
    id: 'standard',
    name: '示例 1 · RLC 谐振实验 (标准)',
    components: [
      { type: 'V', x: 90, y: 140 },
    ...
    connections: [
      [0, 1, 1, 0], // V 右 → R 左（下边）
      [1, 1, 2, 1], // R 右 → L 右（右边·竖）
      [2, 0, 3, 1], // L 左 → C 右（上边）
      [3, 0, 0, 0], // C 左 → V 左（左边·竖，闭合）
    ],
  },
  {
    id: 'under',
