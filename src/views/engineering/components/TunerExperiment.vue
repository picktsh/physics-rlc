<template>
  <div>
    ...
    <section class="rounded-lg bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow)] mb-4">
      <h2
        class="text-base font-bold leading-normal tracking-[0.5px] text-[var(--app-text)] [font-family:var(--app-font-heading)] border-l-4 border-l-[var(--app-brand)] pl-3 tuner-title mb-4"
      >
        收音机选频
      </h2>
      <p class="text-[color:var(--app-text-muted)] leading-6">
        收音机天线收到的并不是"一个台",而是<b class="text-[color:var(--app-text)]">所有电台载波叠加在一起的混合信号</b>。 串联 RLC
        回路对不同频率呈现不同阻抗(谐振时电流最大),把它接在天线之后,调可变电容 C 使谐振频率
        <b class="text-[color:var(--app-brand)]">f₀</b> 对准某个电台,该台的电流分量就远大于其他台 —— 这一过程就是<b
          class="text-[color:var(--app-text)]"
          >选频</b
      ...
      <div class="flex flex-wrap items-center gap-x-3 gap-y-2 mt-3">
        ...
        <span class="text-[color:var(--app-text-faint)]">→</span>
      ...
      <h3 class="flex items-center gap-2 text-base font-bold text-[color:var(--app-text)] mt-4 mb-3">
        <span class="inline-block w-[3px] h-[15px] rounded-full bg-[var(--app-primary)]"></span>核心公式
      </h3>
      <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
        <div class="formula-card flex flex-col rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] px-4 py-4 text-center">
          <div class="text-xs font-semibold text-[color:var(--app-text-faint)] tracking-[0.14em] mb-3">调谐(选台)</div>
          <div class="overflow-x-auto overflow-y-hidden">
            <div
              class="formula-k min-w-max mx-auto text-[color:var(--app-text)]"
        ...
        <div class="formula-card flex flex-col rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] px-4 py-4 text-center">
          <div class="text-xs font-semibold text-[color:var(--app-text-faint)] tracking-[0.14em] mb-3">回路电流</div>
          <div class="overflow-x-auto overflow-y-hidden">
            <div
              class="formula-k min-w-max mx-auto text-[color:var(--app-text)]"
              style="font-size: 18px"
        ...
        <div class="formula-card flex flex-col rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] px-4 py-4 text-center">
          <div class="text-xs font-semibold text-[color:var(--app-text-faint)] tracking-[0.14em] mb-3">品质因数与带宽</div>
          <div
            class="formula-k flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-[color:var(--app-text)]"
            style="font-size: 18px"
          >
        ...
        <div class="formula-card flex flex-col rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] px-4 py-4 text-center">
          <div class="text-xs font-semibold text-[color:var(--app-text-faint)] tracking-[0.14em] mb-3">选频曲线(相对电流)</div>
          <div class="overflow-x-auto overflow-y-hidden">
            <div
              class="formula-k min-w-max mx-auto text-[color:var(--app-text)]"
              style="font-size: 18px"
        ...
        <div class="formula-card flex flex-col rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] px-4 py-4 text-center">
          <div class="text-xs font-semibold text-[color:var(--app-text-faint)] tracking-[0.14em] mb-3">邻台串扰(dB)</div>
          <div class="overflow-x-auto overflow-y-hidden">
            <div
              class="formula-k min-w-max mx-auto text-[color:var(--app-text)]"
              style="font-size: 18px"
      ...
      <p class="text-[color:var(--app-text-faint)] leading-6 mt-3">
    ...
    <section class="rounded-lg bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow)] mb-4">
      <h2
        class="text-18px font-bold leading-normal tracking-[0.5px] text-[var(--app-text)] [font-family:var(--app-font-heading)] border-l-4 border-l-[var(--app-brand)] pl-3 mb-4"
      >
        电台与调谐
      </h2>
      <div class="flex flex-wrap items-center gap-x-4 gap-y-2.5">
        <span class="text-[color:var(--app-text-muted)]">电台(点击直接收听)</span>
        <NButton
          v-for="s in STATIONS"
          :key="s.id"
          secondary
          :type="targetId === s.id ? 'primary' : 'default'"
          @click="tuneTo(s.freq)"
        >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-4 mt-4">
        <div class="flex flex-col gap-3">
          ...
          <div class="flex items-center gap-3 text-[color:var(--app-text-muted)]">
            ...
            <NSlider v-model:value="R" :min="R_MIN" :max="R_MAX" :step="1" class="flex-1" />
            <span class="w-[108px] shrink-0 text-right font-variant-numeric tabular-nums text-[color:var(--app-text)]"
              >{{ R }} Ω</span
            >
          </div>
          <div
          ...
          <div class="px-4 py-3 flex flex-col gap-3">
            <div>
              <div class="flex justify-between mb-1">
                <span class="text-[color:var(--app-text-muted)]">接收电平(相对对准)</span>
              ...
              <div class="text-xs text-[color:var(--app-text-faint)] mt-0.5">{{ levelHint }}</div>
            </div>
            <div>
              <div class="flex justify-between mb-1">
                <span class="text-[color:var(--app-text-muted)]">邻台串扰(最大)</span>
                <span class="font-semibold font-variant-numeric tabular-nums text-[color:var(--app-text)]">{{
    ...
    <section class="rounded-lg bg-[var(--app-surface)] p-4 shadow-[var(--app-shadow)] mb-4">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[1.6fr_1fr] gap-4">
        <div class="rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] p-4 min-w-0">
          <div class="flex flex-wrap items-center gap-x-4 gap-y-1 text-[color:var(--app-text-muted)] mb-2">
            <span class="flex items-center gap-2"
              ><i class="inline-block w-4 border-t-2 border-[color:var(--app-primary)]"></i>选频曲线(相对电流)</span
            >
            <span class="flex items-center gap-2"
              ><i class="inline-block w-2.5 h-2.5 rounded-full bg-[var(--app-warning)]"></i>电台位置</span
            >
            <span class="flex items-center gap-2"
        ...
        <div class="flex flex-col gap-3 min-w-0">
          <div class="rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] p-4 flex-1">
            <div class="font-semibold text-[color:var(--app-text)] mb-2">天线输入频谱(发射强度)</div>
            <div ref="inSpecWrapRef" class="w-full">
              <canvas ref="inSpecCanvasRef" class="w-full block" style="height: 108px"></canvas>
            </div>
            <div class="text-xs text-[color:var(--app-text-faint)] mt-1.5">各台强度相当,谁都不占优</div>
          </div>
          <div class="rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] p-4 flex-1">
            <div class="font-semibold text-[color:var(--app-text)] mb-2">选频后输出(回路电流)</div>
            <div ref="outSpecWrapRef" class="w-full">
              <canvas ref="outSpecCanvasRef" class="w-full block" style="height: 108px"></canvas>
            </div>
            <div class="text-xs text-[color:var(--app-text-faint)] mt-1.5">只剩目标台分量,其余被衰减(数值为相对目标台的 dB)</div>
          </div>
        </div>
      </div>
    </section>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div class="rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] p-4 min-w-0">
          <div class="flex items-center justify-between mb-2">
            <span class="font-semibold text-[color:var(--app-text)]">天线输入:多台叠加</span>
            <span class="text-[color:var(--app-text-faint)]">几个不同频率的载波叠在一起,无法直接收听</span>
          </div>
        ...
        <div class="rounded-lg bg-[var(--app-surface-sunken)] border border-[color:var(--app-border)] p-4 min-w-0">
          <div class="flex items-center justify-between mb-2">
            <span class="font-semibold text-[color:var(--app-text)]">选频输出:回路电流</span>
            <span class="text-[color:var(--app-text-faint)]">接近目标台的单频波形 —— 可以解调了</span>
          </div>
    ...
    <!-- ============ 5. 注脚 ============ -->
    <p class="text-[color:var(--app-text-faint)] leading-6 px-0.5">
...
const levelColor = computed(() => {
  const h = st.value.target.h
  if (h >= 0.89) return 'text-[color:var(--app-success)]'
  if (h >= 0.5) return 'text-[color:var(--app-warning)]'
  return 'text-[color:var(--app-error)]'
})
...
.chip-flow {
  ...
  border: 1px solid var(--app-surface-brand-strong);
  border-radius: 999px;
  padding: 3px 12px;
  font-weight: 600;
  white-space: nowrap;
