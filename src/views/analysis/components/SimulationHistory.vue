<template>
  <div class="simulation-history">
    ...
    <div class="flex items-center gap-3 mb-3 flex-wrap">
      ...
      <button @click="$emit('export')" class="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-xs hover:bg-gray-300 transition-all" title="保存历史记录为JSON文件"><NIcon :component="Save" /> 保存</button>
      <label class="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-xs hover:bg-gray-300 transition-all cursor-pointer" title="从JSON文件加载历史记录">
        <NIcon :component="FolderOpen" /> 打开
        <input type="file" accept=".json" class="hidden" @change="$emit('import', $event.target.files[0])" />
      </label>
      <button @click="$emit('clear')" class="ml-auto inline-flex items-center gap-1 px-3 py-1.5 bg-gray-200 text-gray-700 rounded text-xs hover:bg-gray-300 transition-all"><NIcon :component="TrashCan" /> 清空记录</button>
    </div>
