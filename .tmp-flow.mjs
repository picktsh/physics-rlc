/* 无头验证:§5 纵向图注流 —— 台与灰字同左缘紧连、字号、无裁切、多档位 */
  ...
  for (const w of [1440, 1280, 1100, 768]) {
    ...
    const rr = await evaluate(conn, `(() => {
      ...
      const rows = [...sec.querySelectorAll('[class*="divide-y"] > div')].map((row) => {
        ...
        const sym = row.querySelector('.formula-k:not([class*="f7f9fc"])')
        const pRect = p.getBoundingClientRect()
        const tRect = tray.getBoundingClientRect()
        const cRect = chart.getBoundingClientRect()
        const sRect = sym.getBoundingClientRect()
        ...
        return {
          ...
          symSize: getComputedStyle(sym).fontSize,
          symBottom: Math.round(sRect.bottom - rowRect.top),
