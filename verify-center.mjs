// verify composition symmetry after placing components (camera must frame origin)
...
const edge = spawn(EDGE, [
  ...
  '--user-data-dir=' + process.env.TEMP + '\\qoder-edge-tmpD',
...
async function grid() {
  ...
  const grid = await evalJS(`(async () => {
    const i = new Image()
    i.src = 'data:image/png;base64,${shot.result.data}'
    await i.decode()
    const W = 112, H = 108
...
console.log('--- after 4 components ---')
await grid()
ws.close()
edge.kill()
process.exit(0)
