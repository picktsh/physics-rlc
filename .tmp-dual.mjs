  ...
  for (const w of [1440, 1280, 1100, 768]) {
    ...
    const rr = await evaluate(conn, `(() => {
      ...
      const rows = [...sec.querySelectorAll('[class*="divide-y"] > div')].map((row) => {
        ...
        const title = row.querySelector('div > div > div.flex').textContent.trim().slice(0, 8)
        const nameEl = row.querySelector('.text-\\\\[16px\\\\]')
        const symEl = row.querySelector('[class*="formula-k"][class*="15px"]')
        ...
        const l = chartRect ? Math.round(chartRect.left - rowRect.left) : -1
        const r = trayRect ? Math.round(rowRect.right - trayRect.right) : -1
        return {
          title,
          nameSize: nameEl ? getComputedStyle(nameEl).fontSize : null,
          symSize: symEl ? getComputedStyle(symEl).fontSize : null,
          pSize: pStyle ? pStyle.fontSize : null,
          pLines: p ? Math.round(p.getBoundingClientRect().height / parseFloat(pStyle.lineHeight)) : -1,
          trayOverflow: tray ? tray.scrollWidth - tray.clientWidth : -1,
          trayW: trayRect ? Math.round(trayRect.width) : -1,
          chartW: chartRect ? Math.round(chartRect.width) : -1,
          chartLeftGap: l,
          trayRightGap: r,
          colGap: chartRect && trayRect ? Math.round(trayRect.left - chartRect.right) : -1,
